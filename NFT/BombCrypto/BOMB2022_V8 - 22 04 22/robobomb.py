...
if __name__ == '__main__': 
    ...
    config_house_zoom_100 = config['config_house_zoom_100']
    ...
    workgreen2 = cv2.imread('targets/new'+past_name+'/heroes/workgreen2.png')#OK
...
def findTelaXY(find_in_inscreen,onclicked= False,doubleclick = False,addX = 0,addY = 0,f5Pag = False,listUn = False,workheroes = False,threshold = 0.85):#BUSCA EM TODA A TELA A POSIÇÃO DAS IMAGENS
    ...
    if(len(pcenter)>0):
        ...
        for (pini,pcenter) in zip(pini,pcenter):
            ...
            if(onclicked):
                pyautogui.moveTo(xc+addX+correc_bug_tela['x'],yc+addY+correc_bug_tela['y'],0.2)
...
movPoint = False,movPointIniaddX = 0, movPointIniaddY = 0, movPointEndaddX = 0, movPointEndaddY = 0,qtdRepeatChk = 1,ListUni = False,goGreen = False, goRed = False,adcWork = False): #BUSCA TODOS AS BOX COM PONTOS CARACTERÍSTICOS
    ...
    if(len(piniref)>0):
        ...
        return False
##########################################################################################
def herosFullWorkUno():
    if(findTelaXY(go_hero_work_ini)):
        print('INICIANDO FULL - APÓS TENTATIVA DE LOGIN ÀS {} '.format(horarioexato()))
        #time.sleep(1)

        if(heroesWindowSelect()):
...
def CheckLogin():
    ...
    while True:
        ...
        if(findTelaXY(loaderro_bt)):
            ...
            print('O BOMBER DEVE ESTAR EM MANUTENÇÃO - IREMOS VERIFICAR NOVAMENTE - CTRL + SHIFT + R - AGUARDE 12s')
            time.sleep(12)
            continue
        ...
        if (findTelaXY(ok_bt)):
            time.sleep(1)
            print('CLICANDO NO OK - NOTA DE ERRO.  AGUARDE UM MOMENTO - CTRL + SHIFT + R')
        ...
        if (findTelaXY(go_conect) or findTelaXY(go_conect2) or findTelaXY(go_conect_meta)):
            time.sleep(1)
            print('ENCONTRADO BOTÃO LOGIN/CONECTED .VAMOS CLICAR NO BOTÃO CONECTED --> LOGIN (ESCOLINHA)')
            time.sleep(1)
            #print('VAMOS CLICAR NO BOTÃO CONECTED --> LOGIN (ESCOLINHA)')
            print('SE EXISTIR VÁRIOS MAPAS (MULT ACOUNT) SERÃO ATUALIZADOS - MOTIVO: EVITAR DESLOGAR DAS OUTRAS CONTAS POR INATIVIDADE')
            updateMapaHero()
            time.sleep(1)
...
def heroesWindowSelect(checkback = False):
    if(checkback):
    ...
    time.sleep(1)
    contchk = 1
    while(findTelaXY(tela_sup_ref) == False or contchk <= 15):
        time.sleep(1)
        print('Aguardando Tela Seleção Heros. Tentativa n° '+str(contchk))
        if(findTelaXY(tela_sup_ref)):
            break
...
def heroesSelectHouse2():
    if(heroesWindowSelect(True)):
        ...
        findTelaXY(go_all_work,True)
        pyautogui.moveRel(0,-50)
        #VERIFICAÇÃO GREEW-HEROES
        qtdcheck = 0
        while qtdcheck <= config_house_zoom_100['inhouse_limit_red_chk']:
            qtdcheck = qtdcheck + 1
            #print(str(qtdcheck)+' --- '+str(config_house_zoom_100['inhouse_limit_red_chk']))
            #checkXBackIniPag() #ATUALIZA TODAS AS PAGINAS - VOLTANDO PARA TELA INICIAL
            print('VERIFICANDO A  CONTA A '+str(qtdcheck)+'° VEZ - METODO - CHECK - HOUSE - HEROES')
            ################################### INICIO CHECK  - GREEN - 1° VEZ ################################
            ...
            #VERIFICA 1° VEZ
            time.sleep(1)
            findTelaXY(noworkred0,True,False,257,-7,False,False,False,0.94)
            findTelaXY(noworkred1,True,False,257,-7,False,False,False,0.94)
            #pyautogui.moveRel(100,0)
            ...
            if(qtdcheck <= config_house_zoom_100['inhouse_limit_red_chk']):
                heroesWindowSelect()
                continue
...
def updateMapaHero():
    print('ATUALIZANDO A POSIÇÃO DOS BOMBER HEROES ÀS {} '.format(horarioexato()))
    checkXBackIniPag()
    conttt = 1
    while(findTelaXY(go_hero_work_ini) and conttt < 10):
        findTelaXY(go_map,True)
        conttt = conttt + 1

def newMap():
    print('ENTRANDO EM UM NOVO MAPA')
...
def checkXBackIniPag():
    contt = 1
    while((findTelaXY(go_back) or findTelaXY(x_button)) and contt < 10):
        findTelaXY(go_back,True)
        findTelaXY(x_button,True)
        findTelaXY(x_button,True)
        findTelaXY(go_back,True)
...
def main():
    cont = 0
    #herosFullWorkIni()
    #findTelaXY(go_conect_escolinha, False,False)
    #heroesSelectHouse2()
    #findTelaXY(workgreen,True,False,140,-10,False)
    #print('OK')
    #return
 ###########################AMBIENTE DE TESTE########################################
    while True:
        
        print('BOT BOMBCRYPTO 2022 (UPDATE 22 04 22) - Aguarde 3s...')
        time.sleep(3)
